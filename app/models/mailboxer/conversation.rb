class Mailboxer::Conversation < ActiveRecord::Base
  ...
  has_many :receipts, -> { order 'created_at' }, :through => :messages,  :class_name => "Mailboxer::Receipt"
  ...
  scope :archive, lambda {|participant|
    participant(participant).merge(Mailboxer::Receipt.archive.not_trash.not_deleted)
  }
